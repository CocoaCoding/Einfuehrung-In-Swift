...
// mit einem Protokoll statt eines Datentyps
func outputLocation(objectInfo:ObjectInformationProtocol)
{
    // Aufruf der Methode die im Protokoll
    // deklariert wurde
    let location = objectInfo.getLocation()
    print(location)
    
    // Methode der Klasse kann hier nicht
    // aufgerufen werden
    // let properties = employee.getAllPropertiesAsString()
    // print(properties)
    
    // Aufruf einer Methode, die optional im
    // Protokoll deklariert wurde
    let info
...
// Aufruf der Funktion
outputLocation(objectInfo: employee)

let ide = "Entwicklungsumgebung"
print(ide)
// Elf Zeichen von rechts
print(ide.left(charCount: 11))
// Acht Zeichen von links
print(ide.right(charCount: 8))

// Definition eines Item-Objektes
let extendedItem = Item()
// Aufruf der Funktion
outputLocation(objectInfo: extendedItem)
